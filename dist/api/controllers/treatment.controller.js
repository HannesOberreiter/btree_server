...
class TreatmentController {
    static async get(req, reply) {
        ...
        const query = treatment_model_js_1.Treatment.query()
            .withGraphJoined('[treatment_apiary, type, disease, vet, hive, creator(identifier), editor(identifier)]')
            .where({
            'hive.deleted': false,
    ...
    static async patch(req, reply) {
        ...
        const result = await treatment_model_js_1.Treatment.transaction(async (trx) => {
            return await treatment_model_js_1.Treatment.query(trx)
    ...
    static async post(req, reply) {
        ...
        const result = await treatment_model_js_1.Treatment.transaction(async (trx) => {
            ...
            for (const hive in hives) {
                const res = await treatment_model_js_1.Treatment.query(trx).insert({
                    ...insert,
    ...
    static async updateStatus(req, reply) {
        const body = req.body;
        const result = await treatment_model_js_1.Treatment.transaction(async (trx) => {
            return treatment_model_js_1.Treatment.query(trx)
                .patch({
    ...
    static async batchGet(req, reply) {
        const body = req.body;
        const result = await treatment_model_js_1.Treatment.transaction(async (trx) => {
            const res = await treatment_model_js_1.Treatment.query(trx)
                .findByIds(body.ids)
    ...
    static async batchDelete(req, reply) {
        ...
        const result = await treatment_model_js_1.Treatment.transaction(async (trx) => {
            ...
            if (hardIds.length > 0)
                await treatment_model_js_1.Treatment.query(trx).delete().whereIn('id', hardIds);
            if (softIds.length > 0)
                await treatment_model_js_1.Treatment.query(trx)
                    .patch({
