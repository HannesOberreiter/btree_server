...
class FeedController {
    static async get(req, reply) {
        ...
        const query = feed_model_js_1.Feed.query()
            .withGraphJoined('[feed_apiary, type, hive, creator(identifier), editor(identifier)]')
            .where({
            'hive.deleted': false,
    ...
    static async patch(req, reply) {
        ...
        const result = await feed_model_js_1.Feed.transaction(async (trx) => {
            return await feed_model_js_1.Feed.query(trx)
    ...
    static async post(req, reply) {
        ...
        const result = await feed_model_js_1.Feed.transaction(async (trx) => {
            ...
            for (const hive in hives) {
                const res = await feed_model_js_1.Feed.query(trx).insert({
                    ...insert,
    ...
    static async updateStatus(req, reply) {
        const body = req.body;
        const result = await feed_model_js_1.Feed.transaction(async (trx) => {
            return feed_model_js_1.Feed.query(trx)
                .patch({
    ...
    static async batchGet(req, reply) {
        const body = req.body;
        const result = await feed_model_js_1.Feed.transaction(async (trx) => {
            const res = await feed_model_js_1.Feed.query(trx)
                .findByIds(body.ids)
    ...
    static async batchDelete(req, reply) {
        ...
        const result = await feed_model_js_1.Feed.transaction(async (trx) => {
            ...
            if (hardIds.length > 0)
                await feed_model_js_1.Feed.query(trx).delete().whereIn('id', hardIds);
            if (softIds.length > 0)
                await feed_model_js_1.Feed.query(trx)
                    .patch({
