...
class MovedateController {
    static async get(req, reply) {
        const { order, direction, offset, limit, q, filters } = req.query;
        const query = movedate_model_js_1.Movedate.query()
            .withGraphJoined('[hive, apiary, creator(identifier), editor(identifier)]')
            .where({
    ...
    static async patch(req, reply) {
        ...
        const result = await movedate_model_js_1.Movedate.transaction(async (trx) => {
            return await movedate_model_js_1.Movedate.query(trx)
                .patch({
                ...insert,
    ...
    static async post(req, reply) {
        ...
        const result = await movedate_model_js_1.Movedate.transaction(async (trx) => {
            ...
            for (let i = 0; i < hive_ids.length; i++) {
                const id = hive_ids[i];
                await hive_location_model_js_1.HiveLocation.query()
                    .where({
                    user_id: req.session.user.user_id,
                    hive_id: id,
                })
                    .throwIfNotFound();
                const res = await movedate_model_js_1.Movedate.query(trx).insert({
                    ...insert,
                    hive_id: id,
    ...
    static async updateDate(req, reply) {
        ...
        const result = await movedate_model_js_1.Movedate.transaction(async (trx) => {
            ...
            const ids = await movedate_model_js_1.Movedate.query(trx)
                .select('movedates.id')
                .findByIds(body.ids)
                .leftJoinRelated('apiary')
                .where('user_id', req.session.user.user_id);
            const ids_array = ids.map((elem) => elem.id);
            return movedate_model_js_1.Movedate.query(trx)
                .patch({
    ...
    static async batchGet(req, reply) {
        const body = req.body;
        const result = await movedate_model_js_1.Movedate.transaction(async (trx) => {
            const res = await movedate_model_js_1.Movedate.query(trx)
                .findByIds(body.ids)
    ...
    static async batchDelete(req, reply) {
        ...
        if (result === 0) {
            throw http_errors_1.default.Forbidden('lastMovement'); // this specific key is used in frontend to show a specific error message
        }
        return result;
    }
}
exports.default = MovedateController;
