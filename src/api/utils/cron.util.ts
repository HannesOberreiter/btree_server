...
export const cleanupDatabase = async () => {
  try {
    ...
    return await Company.transaction(async (trx) => {
        ...
        cleanup['CompanyBee'] = await CompanyBee.query(trx).delete().leftJoinRelated('company').leftJoinRelated('user').whereNull('company.id').orWhereNull('user.id');
        cleanup['Company'] = await Company.query(trx).delete().leftJoinRelated('user').whereNull('user.id')
        cleanup['User'] = await User.query(trx).delete().leftJoinRelated('company').whereNull('company.id')

        const forgottenIds = await CompanyBee.query(trx).distinct('bee_id')
        ...
        cleanup['LoginAttemp'] = await LoginAttemp.query(trx).delete().andWhere('time', '<=', lastMonth)
        
        cleanup['Apiary'] = await Apiary.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id')
            .orWhere('deleted', true)
            .andWhere('deleted_at', '<=', lastMonth)
        cleanup['Hive'] = await Hive.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id')
            .orWhere('deleted', true)
            .andWhere('deleted_at', '<=', lastMonth)
        cleanup['Movedate'] = await Movedate.query(trx).delete()
            .leftJoinRelated('hive')
            .leftJoinRelated('apiary')
            .whereNull('hive.id')
            .orWhereNull('apiary.id')
        
        cleanup['Dropbox'] = await Dropbox.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['charge_types'] = await ChargeType.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['checkup_types'] = await CheckupType.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['feed_types'] = await FeedType.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['harvest_types'] = await HarvestType.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['hive_sources'] = await HiveSource.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['hive_types'] = await HiveType.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['hive_types'] = await HiveType.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['queen_matings'] = await QueenMating.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['queen_races'] = await QueenRace.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['rearing_details'] = await RearingDetail.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id'); 
        cleanup['rearing_types'] = await RearingType.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['rearings'] = await Rearing.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['rearing_steps'] = await RearingStep.query(trx).delete()
            .leftJoinRelated('type')
            .leftJoinRelated('detail')
            .whereNull('type.id')
            .orWhereNull('detail.id');
        cleanup['refresh_tokens'] = await RefreshToken.query(trx).delete()
            .leftJoinRelated('company_bee')
            .whereNull('company_bee.id');
        cleanup['scales'] = await Scale.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['scale_data'] = await ScaleData.query(trx).delete()
            .leftJoinRelated('scale')
            .whereNull('scale.id');
        cleanup['treatment_diseases'] = await TreatmentDisease.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['treatment_types'] = await TreatmentType.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['treatment_vets'] = await TreatmentVet.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id');
        cleanup['Feed'] = await Feed.query(trx).delete()
            .leftJoinRelated('hive')
            .whereNull('hive.id')
            .orWhere('feeds.deleted', true)
            .andWhere('feeds.deleted_at', '<=', lastMonth);
        cleanup['Treatment'] = await Treatment.query(trx).delete()
            .leftJoinRelated('hive')
            .whereNull('hive.id')
            .orWhere('treatments.deleted', true)
            .andWhere('treatments.deleted_at', '<=', lastMonth);
        cleanup['Checkup'] = await Checkup.query(trx).delete()
            .leftJoinRelated('hive')
            .whereNull('hive.id')
            .orWhere('checkups.deleted', true)
            .andWhere('checkups.deleted_at', '<=', lastMonth);
        cleanup['Harvest'] = await Harvest.query(trx).delete()
            .leftJoinRelated('hive')
            .whereNull('hive.id')
            .orWhere('harvests.deleted', true)
            .andWhere('harvests.deleted_at', '<=', lastMonth);
        cleanup['Queen'] = await Queen.query(trx).delete()
            .leftJoinRelated('company')
            .whereNull('company.id')
            .orWhere('deleted', true)
            .andWhere('deleted_at', '<=', lastMonth);
