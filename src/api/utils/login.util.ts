...
import { badRequest, expectationFailed, locked, notFound, unauthorized } from "@hapi/boom";
...
const checkBruteForce = async ( bee_id : number) => {
    try {
      ...
      const validAttempts = Moment().hour(-2).format();
      const bruteForce = await LoginAttemp
      .query()
      .count('id as count')
      .where('bee_id', bee_id)
...
const checkPassword = ( inputPassword: string, dbPassword: string, salt: string, hash: string = 'sha512' ) => {
