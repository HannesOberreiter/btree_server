...
import { executeWizBeeTool } from '../../src/api/modules/wizbee_tools.module.js';
...
describe('WizBee tool authorization', () => {
  ...
  it('denies write tools to read-only users', async () => {
    const result = await executeWizBeeTool(
      unusedDb,
      'createMovement',
      { hiveId: 1, apiaryId: 1, date: '2026-01-01' },
      { userId: 1, beeId: 1, rank: 3 },
    );
