...
import {
  listScales,
  updateScales,
} from '../../src/api/modules/scale.module.js';
...
describe('scale routes', () => {
  ...
  describe('/api/v1/scale/', () => {
    ...
    it('get 200 - success', async () => {
      ...
      const scale = res.body.find(
        (item: { id: number }) => item.id === insertId,
      );
      expect(scale).toMatchObject({
        id: insertId,
        name: testInsert2.name,
        hive_id: testInsert2.hive_id,
      });
      expect(scale.hive).toMatchObject({ id: testInsert2.hive_id });
    });

    it('operations enforce company isolation', async () => {
      const db = KyselyServer.getInstance().db;
      expect(await listScales(db, 999_999, insertId)).toEqual([]);
      expect(
        await updateScales(db, 999_999, {
          ids: [insertId],
