...
describe('queen routes', () => {
  ...
  describe('/api/v1/queen/stats', () => {
    it('get 401 - no header', async () => {
      const res = await doQueryRequest(createAgent(), `${route}/stats`, null, null, null);
      expect(res.statusCode).toEqual(401);
      expect(res.errors, 'JsonWebTokenError');
    });

    it('get 200 - success', async () => {
      const res = await doQueryRequest(agent, `${route}/stats`, null, accessToken, null);
      expect(res.statusCode).toEqual(200);
      expect(res.body.results).toBeInstanceOf(Array);
      expect(res.body.total).toBeTypeOf('number');
    });
  ...
  describe('/api/v1/queen/pedigree/:id', () => {
    it('get 401 - no header', async () => {
      const res = await doQueryRequest(createAgent(), `${route}/pedigree/7850`, null, null, null);
      expect(res.statusCode).toEqual(401);
      expect(res.errors, 'JsonWebTokenError');
    });

    it('get 200 - success', async () => {
      const res = await doQueryRequest(agent, `${route}/pedigree/7850`, null, accessToken, null);
      expect(res.statusCode).toEqual(200);
