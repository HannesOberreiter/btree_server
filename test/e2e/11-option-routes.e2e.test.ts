...
options.forEach((option) => {
  describe(`${option} routes`, () => {
    ...
    describe(`/api/v1/option/${option}/favorite`, () => {
      it('401 - no header', async () => {
        const res = await doRequest(createAgent(), 'patch', `${route}/favorite`, null, null, { ids: [] });
        expect(res.statusCode).toEqual(401);
        expect(res.errors, 'JsonWebTokenError');
      });
      it('400 - missing ids', async () => {
        const res = await doRequest(agent, 'patch', `${route}/favorite`, null, null, null);
        expect(res.statusCode).toEqual(400);
        expectations(res, 'ids', 'Invalid value');
      });
      it('200 - success', async () => {
        const res = await doRequest(agent, 'patch', `${route}/favorite`, null, accessToken, { ids: [insertId] });
        expect(res.statusCode).toEqual(200);
        expect(res.body).toBe(1);
      });
    });
